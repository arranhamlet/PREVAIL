...
// [[dust2::parameter(S0, type = "int", rank = 3, required = TRUE, constant = FALSE)]]
// [[dust2::parameter(I0, type = "int", rank = 3, required = TRUE, constant = FALSE)]]
// [[dust2::parameter(Rpop0, type = "int", rank = 3, required = TRUE, constant = FALSE)]]
...
public:
  ...
  struct internal_state {
    ...
    std::vector<real_type> waning_to_S_short;
  ...
  struct data_type {
    std::vector<real_type> serosurvey;
  };
  ...
  static shared_state build_shared(cpp11::list parameters) {
    ...
    std::vector<int> S0(dim.S0.size);
    dust2::r::read_int_array(parameters, dim.S0, S0.data(), "S0", true);
    std::vector<int> I0(dim.I0.size);
    dust2::r::read_int_array(parameters, dim.I0, I0.data(), "I0", true);
    std::vector<int> Rpop0(dim.Rpop0.size);
    dust2::r::read_int_array(parameters, dim.Rpop0, Rpop0.data(), "Rpop0", true);
  ...
  static internal_state build_internal(const shared_state& shared) {
    ...
    std::vector<real_type> waning_to_S_short(shared.dim.waning_to_S_short.size);
    ...
    return internal_state{Npop_age_risk, vaccinated_mums, antibody_mums, aging_into_E, aging_into_I, aging_into_R, aging_into_Is, aging_into_Rc, aging_out_of_E, aging_out_of_I, aging_out_of_R, aging_out_of_Is, aging_out_of_Rc, migration, migration_distribution, death_int, repro_weight_now, birth_int, vaccination_prop, t_seeded, E_after_aging, I_after_aging, R_after_aging, Is_after_aging, Rc_after_aging, background_death, reproductive_population, seeded_actual, waning_R, waning_Rc, vaccinating_out_of_E, vaccinating_out_of_I, vaccinating_out_of_R, vaccinating_out_of_Is, vaccinating_out_of_Rc, migration_adjusted, Npop_background_death, prop_maternal_vaccinated, prop_maternal_natural, infectious_period, vaccinating_into_E, vaccinating_into_I, vaccinating_into_R, vaccinating_into_Is, vaccinating_into_Rc, birth_rate, beta, E_after_vaccination, I_after_vaccination, R_after_vaccination, Is_after_vaccination, Rc_after_vaccination, migration_S, migration_E, migration_I, migration_R, migration_Is, migration_Rc, Births, beta_updated, aging_into_S, waning_from_E_short, waning_from_E_long, waning_from_I_short, waning_from_I_long, waning_from_R_short, waning_from_R_long, waning_from_Is_short, waning_from_Is_long, waning_from_Rc_short, waning_from_Rc_long, aging_out_of_S, waning_to_E_long, waning_to_E_unvaccinated, waning_to_I_long, waning_to_I_unvaccinated, waning_to_R_long, waning_to_R_unvaccinated, waning_to_Is_long, waning_to_Is_unvaccinated, waning_to_Rc_long, waning_to_Rc_unvaccinated, S_after_aging, E_after_waning, I_after_waning, R_after_waning, Is_after_waning, Rc_after_waning, E_available, I_available, R_available, Rc_available, Is_available, vaccinating_out_of_S, incubated, recovered_I_to_R, recovered_from_Is, E_death, I_death, R_death, Is_death, Rc_death, vaccinating_into_S, inf_weighted, into_I, recovered_Is_to_R, S_after_vaccination, infectious_source, into_Is, recovered_Is_to_Rc, waning_from_S_short, waning_from_S_long, lambda_contact, waning_to_S_long, waning_to_S_unvaccinated, waning_to_S_short, S_after_waning, S_available, S_death, Npop_age, ngm_unfolded, lambda_raw, ngm, lambda, lambda_S};
  }
  static data_type build_data(cpp11::list data, const shared_state& shared) {
    ...
    dust2::r::read_real_array(data, shared.dim.serosurvey, serosurvey.data(), "serosurvey", true);
    return data_type{serosurvey};
  }
  static void update_shared(cpp11::list parameters, shared_state& shared) {
    ...
    dust2::r::read_int_array(parameters, shared.dim.S0, shared.S0.data(), "S0", false);
    dust2::r::read_int_array(parameters, shared.dim.I0, shared.I0.data(), "I0", false);
    dust2::r::read_int_array(parameters, shared.dim.Rpop0, shared.Rpop0.data(), "Rpop0", false);
  ...
  static void initial(real_type time, const shared_state& shared, internal_state& internal, rng_state_type& rng_state, real_type* state) {
    for (size_t i = 1; i <= shared.dim.S.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.S.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.S.dim[2]; ++k) {
          state[i - 1 + (j - 1) * shared.dim.S.mult[1] + (k - 1) * shared.dim.S.mult[2] + 8] = shared.S0[i - 1 + (j - 1) * shared.dim.S0.mult[1] + (k - 1) * shared.dim.S0.mult[2]];
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.E.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.E.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.E.dim[2]; ++k) {
          state[i - 1 + (j - 1) * shared.dim.E.mult[1] + (k - 1) * shared.dim.E.mult[2] + shared.odin.offset.state[9]] = 0;
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.I.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.I.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.I.dim[2]; ++k) {
          state[i - 1 + (j - 1) * shared.dim.I.mult[1] + (k - 1) * shared.dim.I.mult[2] + shared.odin.offset.state[10]] = shared.I0[i - 1 + (j - 1) * shared.dim.I0.mult[1] + (k - 1) * shared.dim.I0.mult[2]];
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.R.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.R.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.R.dim[2]; ++k) {
          state[i - 1 + (j - 1) * shared.dim.R.mult[1] + (k - 1) * shared.dim.R.mult[2] + shared.odin.offset.state[11]] = shared.Rpop0[i - 1 + (j - 1) * shared.dim.Rpop0.mult[1] + (k - 1) * shared.dim.Rpop0.mult[2]];
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.Is.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.Is.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.Is.dim[2]; ++k) {
          state[i - 1 + (j - 1) * shared.dim.Is.mult[1] + (k - 1) * shared.dim.Is.mult[2] + shared.odin.offset.state[12]] = 0;
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.Rc.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.Rc.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.Rc.dim[2]; ++k) {
          state[i - 1 + (j - 1) * shared.dim.Rc.mult[1] + (k - 1) * shared.dim.Rc.mult[2] + shared.odin.offset.state[13]] = 0;
        }
      }
    }
    state[0] = 0;
    state[1] = shared.R0[0];
    state[2] = 0;
    state[3] = dust2::array::sum<real_type>(shared.S0.data(), shared.dim.S0) + dust2::array::sum<real_type>(shared.I0.data(), shared.dim.I0) + dust2::array::sum<real_type>(shared.Rpop0.data(), shared.dim.Rpop0);
    ...
    for (size_t i = 1; i <= shared.dim.new_case.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.new_case.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.new_case.dim[2]; ++k) {
          state[i - 1 + (j - 1) * shared.dim.new_case.mult[1] + (k - 1) * shared.dim.new_case.mult[2] + shared.odin.offset.state[15]] = shared.I0[i - 1 + (j - 1) * shared.dim.I0.mult[1] + (k - 1) * shared.dim.I0.mult[2]];
        }
      }
    }
  }
  static void update(real_type time, real_type dt, const real_type* state, const shared_state& shared, internal_state& internal, rng_state_type& rng_state, real_type* state_next) {
    const auto * S = state + 8;
    const auto * E = state + shared.odin.offset.state[9];
    const auto * I = state + shared.odin.offset.state[10];
    const auto * R = state + shared.odin.offset.state[11];
    const auto * Is = state + shared.odin.offset.state[12];
    const auto * Rc = state + shared.odin.offset.state[13];
    const auto total_births = state[4];
    const auto total_deaths = state[5];
    ...
    const real_type migration_prop_S = (N <= 0 ? 0 : dust2::array::sum<real_type>(S, shared.dim.S) / N);
    const real_type migration_prop_E = (N <= 0 ? 0 : dust2::array::sum<real_type>(E, shared.dim.E) / N);
    const real_type migration_prop_I = (N <= 0 ? 0 : dust2::array::sum<real_type>(I, shared.dim.I) / N);
    const real_type migration_prop_R = (N <= 0 ? 0 : dust2::array::sum<real_type>(R, shared.dim.R) / N);
    const real_type migration_prop_Is = (N <= 0 ? 0 : dust2::array::sum<real_type>(Is, shared.dim.Is) / N);
    const real_type migration_prop_Rc = (N <= 0 ? 0 : dust2::array::sum<real_type>(Rc, shared.dim.Rc) / N);
    ...
    for (size_t i = 1; i <= shared.dim.E_after_aging.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.E_after_aging.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.E_after_aging.dim[2]; ++k) {
          internal.E_after_aging[i - 1 + (j - 1) * shared.dim.E_after_aging.mult[1] + (k - 1) * shared.dim.E_after_aging.mult[2]] = monty::math::max<real_type>(E[i - 1 + (j - 1) * shared.dim.E.mult[1] + (k - 1) * shared.dim.E.mult[2]] + internal.aging_into_E[i - 1 + (j - 1) * shared.dim.aging_into_E.mult[1] + (k - 1) * shared.dim.aging_into_E.mult[2]] - internal.aging_out_of_E[i - 1 + (j - 1) * shared.dim.aging_out_of_E.mult[1] + (k - 1) * shared.dim.aging_out_of_E.mult[2]], 0);
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.I_after_aging.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.I_after_aging.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.I_after_aging.dim[2]; ++k) {
          internal.I_after_aging[i - 1 + (j - 1) * shared.dim.I_after_aging.mult[1] + (k - 1) * shared.dim.I_after_aging.mult[2]] = monty::math::max<real_type>(I[i - 1 + (j - 1) * shared.dim.I.mult[1] + (k - 1) * shared.dim.I.mult[2]] + internal.aging_into_I[i - 1 + (j - 1) * shared.dim.aging_into_I.mult[1] + (k - 1) * shared.dim.aging_into_I.mult[2]] - internal.aging_out_of_I[i - 1 + (j - 1) * shared.dim.aging_out_of_I.mult[1] + (k - 1) * shared.dim.aging_out_of_I.mult[2]], 0);
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.R_after_aging.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.R_after_aging.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.R_after_aging.dim[2]; ++k) {
          internal.R_after_aging[i - 1 + (j - 1) * shared.dim.R_after_aging.mult[1] + (k - 1) * shared.dim.R_after_aging.mult[2]] = monty::math::max<real_type>(R[i - 1 + (j - 1) * shared.dim.R.mult[1] + (k - 1) * shared.dim.R.mult[2]] + internal.aging_into_R[i - 1 + (j - 1) * shared.dim.aging_into_R.mult[1] + (k - 1) * shared.dim.aging_into_R.mult[2]] - internal.aging_out_of_R[i - 1 + (j - 1) * shared.dim.aging_out_of_R.mult[1] + (k - 1) * shared.dim.aging_out_of_R.mult[2]], 0);
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.Is_after_aging.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.Is_after_aging.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.Is_after_aging.dim[2]; ++k) {
          internal.Is_after_aging[i - 1 + (j - 1) * shared.dim.Is_after_aging.mult[1] + (k - 1) * shared.dim.Is_after_aging.mult[2]] = monty::math::max<real_type>(Is[i - 1 + (j - 1) * shared.dim.Is.mult[1] + (k - 1) * shared.dim.Is.mult[2]] + internal.aging_into_Is[i - 1 + (j - 1) * shared.dim.aging_into_Is.mult[1] + (k - 1) * shared.dim.aging_into_Is.mult[2]] - internal.aging_out_of_Is[i - 1 + (j - 1) * shared.dim.aging_out_of_Is.mult[1] + (k - 1) * shared.dim.aging_out_of_Is.mult[2]], 0);
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.Rc_after_aging.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.Rc_after_aging.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.Rc_after_aging.dim[2]; ++k) {
          internal.Rc_after_aging[i - 1 + (j - 1) * shared.dim.Rc_after_aging.mult[1] + (k - 1) * shared.dim.Rc_after_aging.mult[2]] = monty::math::max<real_type>(Rc[i - 1 + (j - 1) * shared.dim.Rc.mult[1] + (k - 1) * shared.dim.Rc.mult[2]] + internal.aging_into_Rc[i - 1 + (j - 1) * shared.dim.aging_into_Rc.mult[1] + (k - 1) * shared.dim.aging_into_Rc.mult[2]] - internal.aging_out_of_Rc[i - 1 + (j - 1) * shared.dim.aging_out_of_Rc.mult[1] + (k - 1) * shared.dim.aging_out_of_Rc.mult[2]], 0);
        }
      }
    }
    ...
    for (size_t i = 1; i <= shared.dim.waning_Rc.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.waning_Rc.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.waning_Rc.dim[2]; ++k) {
          internal.waning_Rc[i - 1 + (j - 1) * shared.dim.waning_Rc.mult[1] + (k - 1) * shared.dim.waning_Rc.mult[2]] = (internal.Rc_after_aging[i - 1 + (j - 1) * shared.dim.Rc_after_aging.mult[1] + (k - 1) * shared.dim.Rc_after_aging.mult[2]] <= 0 ? 0 : monty::random::binomial<real_type>(rng_state, internal.Rc_after_aging[i - 1 + (j - 1) * shared.dim.Rc_after_aging.mult[1] + (k - 1) * shared.dim.Rc_after_aging.mult[2]], monty::math::max<real_type>(monty::math::min<real_type>(shared.natural_immunity_waning, 1), 0)));
        }
      }
    }
    ...
    for (size_t i = 1; i <= shared.dim.vaccinating_into_E.dim[0]; ++i) {
      for (size_t j = 1; j <= 2; ++j) {
        for (size_t k = 1; k <= shared.dim.vaccinating_into_E.dim[2]; ++k) {
          internal.vaccinating_into_E[i - 1 + (j - 1) * shared.dim.vaccinating_into_E.mult[1] + (k - 1) * shared.dim.vaccinating_into_E.mult[2]] = 0;
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.vaccinating_into_E.dim[0]; ++i) {
      for (size_t k = 1; k <= shared.dim.vaccinating_into_E.dim[2]; ++k) {
        internal.vaccinating_into_E[i - 1 + 2 * shared.dim.vaccinating_into_E.mult[1] + (k - 1) * shared.dim.vaccinating_into_E.mult[2]] = internal.vaccinating_out_of_E[i - 1 + (k - 1) * shared.dim.vaccinating_out_of_E.mult[2]];
      }
    }
    for (size_t i = 1; i <= shared.dim.vaccinating_into_E.dim[0]; ++i) {
      for (size_t j = 4; j <= static_cast<size_t>(shared.n_vacc); ++j) {
        for (size_t k = 1; k <= shared.dim.vaccinating_into_E.dim[2]; ++k) {
          internal.vaccinating_into_E[i - 1 + (j - 1) * shared.dim.vaccinating_into_E.mult[1] + (k - 1) * shared.dim.vaccinating_into_E.mult[2]] = (std::fmod(j, 2) == 1 ? internal.vaccinating_out_of_E[i - 1 + (j - 2 - 1) * shared.dim.vaccinating_out_of_E.mult[1] + (k - 1) * shared.dim.vaccinating_out_of_E.mult[2]] + internal.vaccinating_out_of_E[i - 1 + (j - 3 - 1) * shared.dim.vaccinating_out_of_E.mult[1] + (k - 1) * shared.dim.vaccinating_out_of_E.mult[2]] : 0);
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.vaccinating_into_I.dim[0]; ++i) {
      for (size_t j = 1; j <= 2; ++j) {
        for (size_t k = 1; k <= shared.dim.vaccinating_into_I.dim[2]; ++k) {
          internal.vaccinating_into_I[i - 1 + (j - 1) * shared.dim.vaccinating_into_I.mult[1] + (k - 1) * shared.dim.vaccinating_into_I.mult[2]] = 0;
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.vaccinating_into_I.dim[0]; ++i) {
      for (size_t k = 1; k <= shared.dim.vaccinating_into_I.dim[2]; ++k) {
        internal.vaccinating_into_I[i - 1 + 2 * shared.dim.vaccinating_into_I.mult[1] + (k - 1) * shared.dim.vaccinating_into_I.mult[2]] = internal.vaccinating_out_of_I[i - 1 + (k - 1) * shared.dim.vaccinating_out_of_I.mult[2]];
      }
    }
    for (size_t i = 1; i <= shared.dim.vaccinating_into_I.dim[0]; ++i) {
      for (size_t j = 4; j <= static_cast<size_t>(shared.n_vacc); ++j) {
        for (size_t k = 1; k <= shared.dim.vaccinating_into_I.dim[2]; ++k) {
          internal.vaccinating_into_I[i - 1 + (j - 1) * shared.dim.vaccinating_into_I.mult[1] + (k - 1) * shared.dim.vaccinating_into_I.mult[2]] = (std::fmod(j, 2) == 1 ? internal.vaccinating_out_of_I[i - 1 + (j - 2 - 1) * shared.dim.vaccinating_out_of_I.mult[1] + (k - 1) * shared.dim.vaccinating_out_of_I.mult[2]] + internal.vaccinating_out_of_I[i - 1 + (j - 3 - 1) * shared.dim.vaccinating_out_of_I.mult[1] + (k - 1) * shared.dim.vaccinating_out_of_I.mult[2]] : 0);
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.vaccinating_into_R.dim[0]; ++i) {
      for (size_t j = 1; j <= 2; ++j) {
        for (size_t k = 1; k <= shared.dim.vaccinating_into_R.dim[2]; ++k) {
          internal.vaccinating_into_R[i - 1 + (j - 1) * shared.dim.vaccinating_into_R.mult[1] + (k - 1) * shared.dim.vaccinating_into_R.mult[2]] = 0;
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.vaccinating_into_R.dim[0]; ++i) {
      for (size_t k = 1; k <= shared.dim.vaccinating_into_R.dim[2]; ++k) {
        internal.vaccinating_into_R[i - 1 + 2 * shared.dim.vaccinating_into_R.mult[1] + (k - 1) * shared.dim.vaccinating_into_R.mult[2]] = internal.vaccinating_out_of_R[i - 1 + (k - 1) * shared.dim.vaccinating_out_of_R.mult[2]];
      }
    }
    for (size_t i = 1; i <= shared.dim.vaccinating_into_R.dim[0]; ++i) {
      for (size_t j = 4; j <= static_cast<size_t>(shared.n_vacc); ++j) {
        for (size_t k = 1; k <= shared.dim.vaccinating_into_R.dim[2]; ++k) {
          internal.vaccinating_into_R[i - 1 + (j - 1) * shared.dim.vaccinating_into_R.mult[1] + (k - 1) * shared.dim.vaccinating_into_R.mult[2]] = (std::fmod(j, 2) == 1 ? internal.vaccinating_out_of_R[i - 1 + (j - 2 - 1) * shared.dim.vaccinating_out_of_R.mult[1] + (k - 1) * shared.dim.vaccinating_out_of_R.mult[2]] + internal.vaccinating_out_of_R[i - 1 + (j - 3 - 1) * shared.dim.vaccinating_out_of_R.mult[1] + (k - 1) * shared.dim.vaccinating_out_of_R.mult[2]] : 0);
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.vaccinating_into_Is.dim[0]; ++i) {
      for (size_t j = 1; j <= 2; ++j) {
        for (size_t k = 1; k <= shared.dim.vaccinating_into_Is.dim[2]; ++k) {
          internal.vaccinating_into_Is[i - 1 + (j - 1) * shared.dim.vaccinating_into_Is.mult[1] + (k - 1) * shared.dim.vaccinating_into_Is.mult[2]] = 0;
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.vaccinating_into_Is.dim[0]; ++i) {
      for (size_t k = 1; k <= shared.dim.vaccinating_into_Is.dim[2]; ++k) {
        internal.vaccinating_into_Is[i - 1 + 2 * shared.dim.vaccinating_into_Is.mult[1] + (k - 1) * shared.dim.vaccinating_into_Is.mult[2]] = internal.vaccinating_out_of_Is[i - 1 + (k - 1) * shared.dim.vaccinating_out_of_Is.mult[2]];
      }
    }
    for (size_t i = 1; i <= shared.dim.vaccinating_into_Is.dim[0]; ++i) {
      for (size_t j = 4; j <= static_cast<size_t>(shared.n_vacc); ++j) {
        for (size_t k = 1; k <= shared.dim.vaccinating_into_Is.dim[2]; ++k) {
          internal.vaccinating_into_Is[i - 1 + (j - 1) * shared.dim.vaccinating_into_Is.mult[1] + (k - 1) * shared.dim.vaccinating_into_Is.mult[2]] = (std::fmod(j, 2) == 1 ? internal.vaccinating_out_of_Is[i - 1 + (j - 2 - 1) * shared.dim.vaccinating_out_of_Is.mult[1] + (k - 1) * shared.dim.vaccinating_out_of_Is.mult[2]] + internal.vaccinating_out_of_Is[i - 1 + (j - 3 - 1) * shared.dim.vaccinating_out_of_Is.mult[1] + (k - 1) * shared.dim.vaccinating_out_of_Is.mult[2]] : 0);
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.vaccinating_into_Rc.dim[0]; ++i) {
      for (size_t j = 1; j <= 2; ++j) {
        for (size_t k = 1; k <= shared.dim.vaccinating_into_Rc.dim[2]; ++k) {
          internal.vaccinating_into_Rc[i - 1 + (j - 1) * shared.dim.vaccinating_into_Rc.mult[1] + (k - 1) * shared.dim.vaccinating_into_Rc.mult[2]] = 0;
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.vaccinating_into_Rc.dim[0]; ++i) {
      for (size_t k = 1; k <= shared.dim.vaccinating_into_Rc.dim[2]; ++k) {
        internal.vaccinating_into_Rc[i - 1 + 2 * shared.dim.vaccinating_into_Rc.mult[1] + (k - 1) * shared.dim.vaccinating_into_Rc.mult[2]] = internal.vaccinating_out_of_Rc[i - 1 + (k - 1) * shared.dim.vaccinating_out_of_Rc.mult[2]];
      }
    }
    for (size_t i = 1; i <= shared.dim.vaccinating_into_Rc.dim[0]; ++i) {
      for (size_t j = 4; j <= static_cast<size_t>(shared.n_vacc); ++j) {
        for (size_t k = 1; k <= shared.dim.vaccinating_into_Rc.dim[2]; ++k) {
          internal.vaccinating_into_Rc[i - 1 + (j - 1) * shared.dim.vaccinating_into_Rc.mult[1] + (k - 1) * shared.dim.vaccinating_into_Rc.mult[2]] = (std::fmod(j, 2) == 1 ? internal.vaccinating_out_of_Rc[i - 1 + (j - 2 - 1) * shared.dim.vaccinating_out_of_Rc.mult[1] + (k - 1) * shared.dim.vaccinating_out_of_Rc.mult[2]] + internal.vaccinating_out_of_Rc[i - 1 + (j - 3 - 1) * shared.dim.vaccinating_out_of_Rc.mult[1] + (k - 1) * shared.dim.vaccinating_out_of_Rc.mult[2]] : 0);
        }
      }
    }
    const real_type migration_occuring_S = (internal.migration_distribution[0] <= 0 || dust2::array::sum<real_type>(S, shared.dim.S) <= 0 ? 0 : monty::random::binomial<real_type>(rng_state, dust2::array::sum<real_type>(internal.migration_adjusted.data(), shared.dim.migration_adjusted), migration_prop_S));
    const real_type migration_occuring_E = (internal.migration_distribution[0] <= 0 || dust2::array::sum<real_type>(E, shared.dim.E) <= 0 ? 0 : monty::random::binomial<real_type>(rng_state, dust2::array::sum<real_type>(internal.migration_adjusted.data(), shared.dim.migration_adjusted), migration_prop_E));
    const real_type migration_occuring_I = (internal.migration_distribution[0] <= 0 || dust2::array::sum<real_type>(I, shared.dim.I) <= 0 ? 0 : monty::random::binomial<real_type>(rng_state, dust2::array::sum<real_type>(internal.migration_adjusted.data(), shared.dim.migration_adjusted), migration_prop_I));
    const real_type migration_occuring_R = (internal.migration_distribution[0] <= 0 || dust2::array::sum<real_type>(R, shared.dim.R) <= 0 ? 0 : monty::random::binomial<real_type>(rng_state, dust2::array::sum<real_type>(internal.migration_adjusted.data(), shared.dim.migration_adjusted), migration_prop_R));
    const real_type migration_occuring_Is = (internal.migration_distribution[0] <= 0 || dust2::array::sum<real_type>(Is, shared.dim.Is) <= 0 ? 0 : monty::random::binomial<real_type>(rng_state, dust2::array::sum<real_type>(internal.migration_adjusted.data(), shared.dim.migration_adjusted), migration_prop_Is));
    const real_type migration_occuring_Rc = (internal.migration_distribution[0] <= 0 || dust2::array::sum<real_type>(Rc, shared.dim.Rc) <= 0 ? 0 : monty::random::binomial<real_type>(rng_state, dust2::array::sum<real_type>(internal.migration_adjusted.data(), shared.dim.migration_adjusted), migration_prop_Rc));
    ...
    for (size_t i = 1; i <= shared.dim.migration_S.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.migration_S.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.migration_S.dim[2]; ++k) {
          internal.migration_S[i - 1 + (j - 1) * shared.dim.migration_S.mult[1] + (k - 1) * shared.dim.migration_S.mult[2]] = (migration_occuring_S <= 0 || dust2::array::sum<real_type>(S, shared.dim.S) <= 0 ? 0 : monty::random::binomial<real_type>(rng_state, migration_occuring_S, S[i - 1 + (j - 1) * shared.dim.S.mult[1] + (k - 1) * shared.dim.S.mult[2]] / dust2::array::sum<real_type>(S, shared.dim.S)));
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.migration_E.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.migration_E.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.migration_E.dim[2]; ++k) {
          internal.migration_E[i - 1 + (j - 1) * shared.dim.migration_E.mult[1] + (k - 1) * shared.dim.migration_E.mult[2]] = (migration_occuring_E <= 0 || dust2::array::sum<real_type>(E, shared.dim.E) <= 0 ? 0 : monty::random::binomial<real_type>(rng_state, migration_occuring_E, E[i - 1 + (j - 1) * shared.dim.E.mult[1] + (k - 1) * shared.dim.E.mult[2]] / dust2::array::sum<real_type>(E, shared.dim.E)));
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.migration_I.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.migration_I.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.migration_I.dim[2]; ++k) {
          internal.migration_I[i - 1 + (j - 1) * shared.dim.migration_I.mult[1] + (k - 1) * shared.dim.migration_I.mult[2]] = (migration_occuring_I <= 0 || dust2::array::sum<real_type>(I, shared.dim.I) <= 0 ? 0 : monty::random::binomial<real_type>(rng_state, migration_occuring_I, I[i - 1 + (j - 1) * shared.dim.I.mult[1] + (k - 1) * shared.dim.I.mult[2]] / dust2::array::sum<real_type>(I, shared.dim.I)));
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.migration_R.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.migration_R.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.migration_R.dim[2]; ++k) {
          internal.migration_R[i - 1 + (j - 1) * shared.dim.migration_R.mult[1] + (k - 1) * shared.dim.migration_R.mult[2]] = (migration_occuring_R <= 0 || dust2::array::sum<real_type>(R, shared.dim.R) <= 0 ? 0 : monty::random::binomial<real_type>(rng_state, migration_occuring_R, R[i - 1 + (j - 1) * shared.dim.R.mult[1] + (k - 1) * shared.dim.R.mult[2]] / dust2::array::sum<real_type>(R, shared.dim.R)));
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.migration_Is.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.migration_Is.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.migration_Is.dim[2]; ++k) {
          internal.migration_Is[i - 1 + (j - 1) * shared.dim.migration_Is.mult[1] + (k - 1) * shared.dim.migration_Is.mult[2]] = (migration_occuring_Is <= 0 || dust2::array::sum<real_type>(Is, shared.dim.Is) <= 0 ? 0 : monty::random::binomial<real_type>(rng_state, migration_occuring_Is, Is[i - 1 + (j - 1) * shared.dim.Is.mult[1] + (k - 1) * shared.dim.Is.mult[2]] / dust2::array::sum<real_type>(Is, shared.dim.Is)));
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.migration_Rc.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.migration_Rc.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.migration_Rc.dim[2]; ++k) {
          internal.migration_Rc[i - 1 + (j - 1) * shared.dim.migration_Rc.mult[1] + (k - 1) * shared.dim.migration_Rc.mult[2]] = (migration_occuring_Rc <= 0 ? 0 : monty::random::binomial<real_type>(rng_state, migration_occuring_Rc, Rc[i - 1 + (j - 1) * shared.dim.Rc.mult[1] + (k - 1) * shared.dim.Rc.mult[2]] / dust2::array::sum<real_type>(Rc, shared.dim.Rc)));
        }
      }
    }
    ...
    for (size_t i = 1; i <= shared.dim.waning_from_I_short.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.waning_from_I_short.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.waning_from_I_short.dim[2]; ++k) {
          internal.waning_from_I_short[i - 1 + (j - 1) * shared.dim.waning_from_I_short.mult[1] + (k - 1) * shared.dim.waning_from_I_short.mult[2]] = (std::fmod(j, 2) == 1 && j > 1 && internal.I_after_vaccination[i - 1 + (j - 1) * shared.dim.I_after_vaccination.mult[1] + (k - 1) * shared.dim.I_after_vaccination.mult[2]] > 0 ? monty::random::binomial<real_type>(rng_state, internal.I_after_vaccination[i - 1 + (j - 1) * shared.dim.I_after_vaccination.mult[1] + (k - 1) * shared.dim.I_after_vaccination.mult[2]], monty::math::max<real_type>(monty::math::min<real_type>(shared.short_term_waning[j - 1], 1), 0)) : 0);
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.waning_from_I_long.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.waning_from_I_long.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.waning_from_I_long.dim[2]; ++k) {
          internal.waning_from_I_long[i - 1 + (j - 1) * shared.dim.waning_from_I_long.mult[1] + (k - 1) * shared.dim.waning_from_I_long.mult[2]] = (std::fmod(j, 2) == 0 && j > 1 && internal.I_after_vaccination[i - 1 + (j - 1) * shared.dim.I_after_vaccination.mult[1] + (k - 1) * shared.dim.I_after_vaccination.mult[2]] > 0 ? monty::random::binomial<real_type>(rng_state, internal.I_after_vaccination[i - 1 + (j - 1) * shared.dim.I_after_vaccination.mult[1] + (k - 1) * shared.dim.I_after_vaccination.mult[2]], monty::math::max<real_type>(monty::math::min<real_type>(shared.long_term_waning[j - 1], 1), 0)) : 0);
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.waning_from_R_short.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.waning_from_R_short.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.waning_from_R_short.dim[2]; ++k) {
          internal.waning_from_R_short[i - 1 + (j - 1) * shared.dim.waning_from_R_short.mult[1] + (k - 1) * shared.dim.waning_from_R_short.mult[2]] = (std::fmod(j, 2) == 1 && j > 1 && internal.R_after_vaccination[i - 1 + (j - 1) * shared.dim.R_after_vaccination.mult[1] + (k - 1) * shared.dim.R_after_vaccination.mult[2]] > 0 ? monty::random::binomial<real_type>(rng_state, internal.R_after_vaccination[i - 1 + (j - 1) * shared.dim.R_after_vaccination.mult[1] + (k - 1) * shared.dim.R_after_vaccination.mult[2]], monty::math::max<real_type>(monty::math::min<real_type>(shared.short_term_waning[j - 1], 1), 0)) : 0);
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.waning_from_R_long.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.waning_from_R_long.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.waning_from_R_long.dim[2]; ++k) {
          internal.waning_from_R_long[i - 1 + (j - 1) * shared.dim.waning_from_R_long.mult[1] + (k - 1) * shared.dim.waning_from_R_long.mult[2]] = (std::fmod(j, 2) == 0 && j > 1 && internal.R_after_vaccination[i - 1 + (j - 1) * shared.dim.R_after_vaccination.mult[1] + (k - 1) * shared.dim.R_after_vaccination.mult[2]] > 0 ? monty::random::binomial<real_type>(rng_state, internal.R_after_vaccination[i - 1 + (j - 1) * shared.dim.R_after_vaccination.mult[1] + (k - 1) * shared.dim.R_after_vaccination.mult[2]], monty::math::max<real_type>(monty::math::min<real_type>(shared.long_term_waning[j - 1], 1), 0)) : 0);
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.waning_from_Is_short.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.waning_from_Is_short.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.waning_from_Is_short.dim[2]; ++k) {
          internal.waning_from_Is_short[i - 1 + (j - 1) * shared.dim.waning_from_Is_short.mult[1] + (k - 1) * shared.dim.waning_from_Is_short.mult[2]] = (std::fmod(j, 2) == 1 && j > 1 && internal.Is_after_vaccination[i - 1 + (j - 1) * shared.dim.Is_after_vaccination.mult[1] + (k - 1) * shared.dim.Is_after_vaccination.mult[2]] > 0 ? monty::random::binomial<real_type>(rng_state, internal.Is_after_vaccination[i - 1 + (j - 1) * shared.dim.Is_after_vaccination.mult[1] + (k - 1) * shared.dim.Is_after_vaccination.mult[2]], monty::math::max<real_type>(monty::math::min<real_type>(shared.short_term_waning[j - 1], 1), 0)) : 0);
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.waning_from_Is_long.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.waning_from_Is_long.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.waning_from_Is_long.dim[2]; ++k) {
          internal.waning_from_Is_long[i - 1 + (j - 1) * shared.dim.waning_from_Is_long.mult[1] + (k - 1) * shared.dim.waning_from_Is_long.mult[2]] = (std::fmod(j, 2) == 0 && j > 1 && internal.Is_after_vaccination[i - 1 + (j - 1) * shared.dim.Is_after_vaccination.mult[1] + (k - 1) * shared.dim.Is_after_vaccination.mult[2]] > 0 ? monty::random::binomial<real_type>(rng_state, internal.Is_after_vaccination[i - 1 + (j - 1) * shared.dim.Is_after_vaccination.mult[1] + (k - 1) * shared.dim.Is_after_vaccination.mult[2]], monty::math::max<real_type>(monty::math::min<real_type>(shared.long_term_waning[j - 1], 1), 0)) : 0);
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.waning_from_Rc_short.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.waning_from_Rc_short.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.waning_from_Rc_short.dim[2]; ++k) {
          internal.waning_from_Rc_short[i - 1 + (j - 1) * shared.dim.waning_from_Rc_short.mult[1] + (k - 1) * shared.dim.waning_from_Rc_short.mult[2]] = (std::fmod(j, 2) == 1 && j > 1 && internal.Rc_after_vaccination[i - 1 + (j - 1) * shared.dim.Rc_after_vaccination.mult[1] + (k - 1) * shared.dim.Rc_after_vaccination.mult[2]] > 0 ? monty::random::binomial<real_type>(rng_state, internal.Rc_after_vaccination[i - 1 + (j - 1) * shared.dim.Rc_after_vaccination.mult[1] + (k - 1) * shared.dim.Rc_after_vaccination.mult[2]], monty::math::max<real_type>(monty::math::min<real_type>(shared.short_term_waning[j - 1], 1), 0)) : 0);
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.waning_from_Rc_long.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.waning_from_Rc_long.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.waning_from_Rc_long.dim[2]; ++k) {
          internal.waning_from_Rc_long[i - 1 + (j - 1) * shared.dim.waning_from_Rc_long.mult[1] + (k - 1) * shared.dim.waning_from_Rc_long.mult[2]] = (std::fmod(j, 2) == 0 && j > 1 && internal.Rc_after_vaccination[i - 1 + (j - 1) * shared.dim.Rc_after_vaccination.mult[1] + (k - 1) * shared.dim.Rc_after_vaccination.mult[2]] > 0 ? monty::random::binomial<real_type>(rng_state, internal.Rc_after_vaccination[i - 1 + (j - 1) * shared.dim.Rc_after_vaccination.mult[1] + (k - 1) * shared.dim.Rc_after_vaccination.mult[2]], monty::math::max<real_type>(monty::math::min<real_type>(shared.long_term_waning[j - 1], 1), 0)) : 0);
        }
      }
    }
    ...
    for (size_t i = 1; i <= shared.dim.S_after_aging.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.S_after_aging.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.S_after_aging.dim[2]; ++k) {
          internal.S_after_aging[i - 1 + (j - 1) * shared.dim.S_after_aging.mult[1] + (k - 1) * shared.dim.S_after_aging.mult[2]] = monty::math::max<real_type>(S[i - 1 + (j - 1) * shared.dim.S.mult[1] + (k - 1) * shared.dim.S.mult[2]] + internal.aging_into_S[i - 1 + (j - 1) * shared.dim.aging_into_S.mult[1] + (k - 1) * shared.dim.aging_into_S.mult[2]] - internal.aging_out_of_S[i - 1 + (j - 1) * shared.dim.aging_out_of_S.mult[1] + (k - 1) * shared.dim.aging_out_of_S.mult[2]], 0);
        }
      }
    }
    ...
    for (size_t i = 1; i <= shared.dim.incubated.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.incubated.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.incubated.dim[2]; ++k) {
          internal.incubated[i - 1 + (j - 1) * shared.dim.incubated.mult[1] + (k - 1) * shared.dim.incubated.mult[2]] = (internal.E_available[i - 1 + (j - 1) * shared.dim.E_available.mult[1] + (k - 1) * shared.dim.E_available.mult[2]] <= 0 ? 0 : monty::random::binomial<real_type>(rng_state, internal.E_available[i - 1 + (j - 1) * shared.dim.E_available.mult[1] + (k - 1) * shared.dim.E_available.mult[2]], monty::math::max<real_type>(monty::math::min<real_type>(shared.incubation_rate, 1), 0)));
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.recovered_I_to_R.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.recovered_I_to_R.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.recovered_I_to_R.dim[2]; ++k) {
          internal.recovered_I_to_R[i - 1 + (j - 1) * shared.dim.recovered_I_to_R.mult[1] + (k - 1) * shared.dim.recovered_I_to_R.mult[2]] = (internal.I_available[i - 1 + (j - 1) * shared.dim.I_available.mult[1] + (k - 1) * shared.dim.I_available.mult[2]] <= 0 ? 0 : monty::random::binomial<real_type>(rng_state, internal.I_available[i - 1 + (j - 1) * shared.dim.I_available.mult[1] + (k - 1) * shared.dim.I_available.mult[2]], monty::math::max<real_type>(monty::math::min<real_type>(shared.recovery_rate, 1), 0)));
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.recovered_from_Is.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.recovered_from_Is.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.recovered_from_Is.dim[2]; ++k) {
          internal.recovered_from_Is[i - 1 + (j - 1) * shared.dim.recovered_from_Is.mult[1] + (k - 1) * shared.dim.recovered_from_Is.mult[2]] = (internal.Is_available[i - 1 + (j - 1) * shared.dim.Is_available.mult[1] + (k - 1) * shared.dim.Is_available.mult[2]] <= 0 ? 0 : monty::random::binomial<real_type>(rng_state, internal.Is_available[i - 1 + (j - 1) * shared.dim.Is_available.mult[1] + (k - 1) * shared.dim.Is_available.mult[2]], monty::math::max<real_type>(monty::math::min<real_type>(shared.severe_recovery_rate, 1), 0)));
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.E_death.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.E_death.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.E_death.dim[2]; ++k) {
          internal.E_death[i - 1 + (j - 1) * shared.dim.E_death.mult[1] + (k - 1) * shared.dim.E_death.mult[2]] = (internal.E_available[i - 1 + (j - 1) * shared.dim.E_available.mult[1] + (k - 1) * shared.dim.E_available.mult[2]] <= 0 ? 0 : monty::random::binomial<real_type>(rng_state, internal.E_available[i - 1 + (j - 1) * shared.dim.E_available.mult[1] + (k - 1) * shared.dim.E_available.mult[2]], monty::math::max<real_type>(monty::math::min<real_type>(internal.background_death[i - 1 + (k - 1) * shared.dim.background_death.mult[1]], 1), 0)));
        }
      }
    }
    ...
    for (size_t i = 1; i <= shared.dim.R_death.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.R_death.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.R_death.dim[2]; ++k) {
          internal.R_death[i - 1 + (j - 1) * shared.dim.R_death.mult[1] + (k - 1) * shared.dim.R_death.mult[2]] = (internal.R_available[i - 1 + (j - 1) * shared.dim.R_available.mult[1] + (k - 1) * shared.dim.R_available.mult[2]] <= 0 ? 0 : monty::random::binomial<real_type>(rng_state, internal.R_available[i - 1 + (j - 1) * shared.dim.R_available.mult[1] + (k - 1) * shared.dim.R_available.mult[2]], monty::math::max<real_type>(monty::math::min<real_type>(internal.background_death[i - 1 + (k - 1) * shared.dim.background_death.mult[1]], 1), 0)));
        }
      }
    }
    ...
    for (size_t i = 1; i <= shared.dim.Rc_death.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.Rc_death.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.Rc_death.dim[2]; ++k) {
          internal.Rc_death[i - 1 + (j - 1) * shared.dim.Rc_death.mult[1] + (k - 1) * shared.dim.Rc_death.mult[2]] = (internal.Rc_available[i - 1 + (j - 1) * shared.dim.Rc_available.mult[1] + (k - 1) * shared.dim.Rc_available.mult[2]] <= 0 ? 0 : monty::random::binomial<real_type>(rng_state, internal.Rc_available[i - 1 + (j - 1) * shared.dim.Rc_available.mult[1] + (k - 1) * shared.dim.Rc_available.mult[2]], monty::math::max<real_type>(monty::math::min<real_type>(internal.background_death[i - 1 + (k - 1) * shared.dim.background_death.mult[1]], 1), 0)));
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.vaccinating_into_S.dim[0]; ++i) {
      for (size_t j = 1; j <= 2; ++j) {
        for (size_t k = 1; k <= shared.dim.vaccinating_into_S.dim[2]; ++k) {
          internal.vaccinating_into_S[i - 1 + (j - 1) * shared.dim.vaccinating_into_S.mult[1] + (k - 1) * shared.dim.vaccinating_into_S.mult[2]] = 0;
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.vaccinating_into_S.dim[0]; ++i) {
      for (size_t k = 1; k <= shared.dim.vaccinating_into_S.dim[2]; ++k) {
        internal.vaccinating_into_S[i - 1 + 2 * shared.dim.vaccinating_into_S.mult[1] + (k - 1) * shared.dim.vaccinating_into_S.mult[2]] = internal.vaccinating_out_of_S[i - 1 + (k - 1) * shared.dim.vaccinating_out_of_S.mult[2]];
      }
    }
    for (size_t i = 1; i <= shared.dim.vaccinating_into_S.dim[0]; ++i) {
      for (size_t j = 4; j <= static_cast<size_t>(shared.n_vacc); ++j) {
        for (size_t k = 1; k <= shared.dim.vaccinating_into_S.dim[2]; ++k) {
          internal.vaccinating_into_S[i - 1 + (j - 1) * shared.dim.vaccinating_into_S.mult[1] + (k - 1) * shared.dim.vaccinating_into_S.mult[2]] = (std::fmod(j, 2) == 1 ? internal.vaccinating_out_of_S[i - 1 + (j - 2 - 1) * shared.dim.vaccinating_out_of_S.mult[1] + (k - 1) * shared.dim.vaccinating_out_of_S.mult[2]] + internal.vaccinating_out_of_S[i - 1 + (j - 3 - 1) * shared.dim.vaccinating_out_of_S.mult[1] + (k - 1) * shared.dim.vaccinating_out_of_S.mult[2]] : 0);
        }
      }
    }
    ...
    for (size_t i = 1; i <= shared.dim.into_I.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.into_I.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.into_I.dim[2]; ++k) {
          internal.into_I[i - 1 + (j - 1) * shared.dim.into_I.mult[1] + (k - 1) * shared.dim.into_I.mult[2]] = (internal.incubated[i - 1 + (j - 1) * shared.dim.incubated.mult[1] + (k - 1) * shared.dim.incubated.mult[2]] <= 0 ? 0 : monty::random::binomial<real_type>(rng_state, internal.incubated[i - 1 + (j - 1) * shared.dim.incubated.mult[1] + (k - 1) * shared.dim.incubated.mult[2]], monty::math::max<real_type>(monty::math::min<real_type>(1 - shared.prop_severe[i - 1 + (j - 1) * shared.dim.prop_severe.mult[1] + (k - 1) * shared.dim.prop_severe.mult[2]], 1), 0)));
        }
      }
    }
    ...
    for (size_t i = 1; i <= shared.dim.recovered_Is_to_Rc.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.recovered_Is_to_Rc.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.recovered_Is_to_Rc.dim[2]; ++k) {
          internal.recovered_Is_to_Rc[i - 1 + (j - 1) * shared.dim.recovered_Is_to_Rc.mult[1] + (k - 1) * shared.dim.recovered_Is_to_Rc.mult[2]] = (internal.recovered_Is_to_R[i - 1 + (j - 1) * shared.dim.recovered_Is_to_R.mult[1] + (k - 1) * shared.dim.recovered_Is_to_R.mult[2]] >= internal.recovered_from_Is[i - 1 + (j - 1) * shared.dim.recovered_from_Is.mult[1] + (k - 1) * shared.dim.recovered_from_Is.mult[2]] ? 0 : internal.recovered_from_Is[i - 1 + (j - 1) * shared.dim.recovered_from_Is.mult[1] + (k - 1) * shared.dim.recovered_from_Is.mult[2]] - internal.recovered_Is_to_R[i - 1 + (j - 1) * shared.dim.recovered_Is_to_R.mult[1] + (k - 1) * shared.dim.recovered_Is_to_R.mult[2]]);
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.waning_from_S_short.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.waning_from_S_short.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.waning_from_S_short.dim[2]; ++k) {
          internal.waning_from_S_short[i - 1 + (j - 1) * shared.dim.waning_from_S_short.mult[1] + (k - 1) * shared.dim.waning_from_S_short.mult[2]] = (std::fmod(j, 2) == 1 && j > 1 && internal.S_after_vaccination[i - 1 + (j - 1) * shared.dim.S_after_vaccination.mult[1] + (k - 1) * shared.dim.S_after_vaccination.mult[2]] > 0 ? monty::random::binomial<real_type>(rng_state, internal.S_after_vaccination[i - 1 + (j - 1) * shared.dim.S_after_vaccination.mult[1] + (k - 1) * shared.dim.S_after_vaccination.mult[2]], monty::math::max<real_type>(monty::math::min<real_type>(shared.short_term_waning[j - 1], 1), 0)) : 0);
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.waning_from_S_long.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.waning_from_S_long.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.waning_from_S_long.dim[2]; ++k) {
          internal.waning_from_S_long[i - 1 + (j - 1) * shared.dim.waning_from_S_long.mult[1] + (k - 1) * shared.dim.waning_from_S_long.mult[2]] = (std::fmod(j, 2) == 0 && j > 1 && static_cast<int>(j) <= shared.n_vacc && internal.S_after_vaccination[i - 1 + (j - 1) * shared.dim.S_after_vaccination.mult[1] + (k - 1) * shared.dim.S_after_vaccination.mult[2]] > 0 ? monty::random::binomial<real_type>(rng_state, internal.S_after_vaccination[i - 1 + (j - 1) * shared.dim.S_after_vaccination.mult[1] + (k - 1) * shared.dim.S_after_vaccination.mult[2]], shared.long_term_waning[j - 1]) : 0);
        }
      }
    }
    ...
    for (size_t i = 1; i <= shared.dim.waning_to_S_unvaccinated.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.waning_to_S_unvaccinated.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.waning_to_S_unvaccinated.dim[2]; ++k) {
          internal.waning_to_S_unvaccinated[i - 1 + (j - 1) * shared.dim.waning_to_S_unvaccinated.mult[1] + (k - 1) * shared.dim.waning_to_S_unvaccinated.mult[2]] = (j == 1 && shared.n_vacc >= 2 ? internal.waning_from_S_long[i - 1 + shared.dim.waning_from_S_long.mult[1] + (k - 1) * shared.dim.waning_from_S_long.mult[2]] : 0);
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.waning_to_S_short.dim[0]; ++i) {
      for (size_t j = 1; j <= static_cast<size_t>((shared.n_vacc - 1)); ++j) {
        for (size_t k = 1; k <= shared.dim.waning_to_S_short.dim[2]; ++k) {
          internal.waning_to_S_short[i - 1 + (j - 1) * shared.dim.waning_to_S_short.mult[1] + (k - 1) * shared.dim.waning_to_S_short.mult[2]] = (std::fmod(j, 2) == 1 && j > 1 && j + 1 <= shared.n_vacc ? internal.waning_from_S_long[i - 1 + (j + 1 - 1) * shared.dim.waning_from_S_long.mult[1] + (k - 1) * shared.dim.waning_from_S_long.mult[2]] : 0);
        }
      }
    }
    ...
    for (size_t i = 1; i <= shared.dim.S_death.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.S_death.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.S_death.dim[2]; ++k) {
          internal.S_death[i - 1 + (j - 1) * shared.dim.S_death.mult[1] + (k - 1) * shared.dim.S_death.mult[2]] = (internal.S_available[i - 1 + (j - 1) * shared.dim.S_available.mult[1] + (k - 1) * shared.dim.S_available.mult[2]] <= 0 ? 0 : monty::random::binomial<real_type>(rng_state, internal.S_available[i - 1 + (j - 1) * shared.dim.S_available.mult[1] + (k - 1) * shared.dim.S_available.mult[2]], monty::math::max<real_type>(monty::math::min<real_type>(internal.background_death[i - 1 + (k - 1) * shared.dim.background_death.mult[1]], 1), 0)));
        }
      }
    }
    ...
    for (size_t i = 1; i <= shared.dim.lambda.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.lambda.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.lambda.dim[2]; ++k) {
          internal.lambda[i - 1 + (j - 1) * shared.dim.lambda.mult[1] + (k - 1) * shared.dim.lambda.mult[2]] = (N <= 0 ? 0 : monty::math::max<real_type>(0, internal.lambda_raw[i - 1]) * (1 - shared.age_vaccination_beta_modifier[i - 1 + (j - 1) * shared.dim.age_vaccination_beta_modifier.mult[1] + (k - 1) * shared.dim.age_vaccination_beta_modifier.mult[2]]));
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.lambda_S.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.lambda_S.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.lambda_S.dim[2]; ++k) {
          internal.lambda_S[i - 1 + (j - 1) * shared.dim.lambda_S.mult[1] + (k - 1) * shared.dim.lambda_S.mult[2]] = (internal.S_available[i - 1 + (j - 1) * shared.dim.S_available.mult[1] + (k - 1) * shared.dim.S_available.mult[2]] <= 0 ? 0 : monty::random::binomial<real_type>(rng_state, internal.S_available[i - 1 + (j - 1) * shared.dim.S_available.mult[1] + (k - 1) * shared.dim.S_available.mult[2]], monty::math::max<real_type>(monty::math::min<real_type>(internal.lambda[i - 1 + (j - 1) * shared.dim.lambda.mult[1] + (k - 1) * shared.dim.lambda.mult[2]], 1), 0)));
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.S.dim[0]; ++i) {
    ...
    for (size_t i = 1; i <= shared.dim.Rc.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.Rc.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.Rc.dim[2]; ++k) {
          state_next[i - 1 + (j - 1) * shared.dim.Rc.mult[1] + (k - 1) * shared.dim.Rc.mult[2] + shared.odin.offset.state[13]] = monty::math::max<real_type>(Rc[i - 1 + (j - 1) * shared.dim.Rc.mult[1] + (k - 1) * shared.dim.Rc.mult[2]] + internal.recovered_Is_to_Rc[i - 1 + (j - 1) * shared.dim.recovered_Is_to_Rc.mult[1] + (k - 1) * shared.dim.recovered_Is_to_Rc.mult[2]] - internal.waning_Rc[i - 1 + (j - 1) * shared.dim.waning_Rc.mult[1] + (k - 1) * shared.dim.waning_Rc.mult[2]] + internal.aging_into_Rc[i - 1 + (j - 1) * shared.dim.aging_into_Rc.mult[1] + (k - 1) * shared.dim.aging_into_Rc.mult[2]] - internal.aging_out_of_Rc[i - 1 + (j - 1) * shared.dim.aging_out_of_Rc.mult[1] + (k - 1) * shared.dim.aging_out_of_Rc.mult[2]] - internal.Rc_death[i - 1 + (j - 1) * shared.dim.Rc_death.mult[1] + (k - 1) * shared.dim.Rc_death.mult[2]] + internal.migration_Rc[i - 1 + (j - 1) * shared.dim.migration_Rc.mult[1] + (k - 1) * shared.dim.migration_Rc.mult[2]] * pos_neg_migration + internal.vaccinating_into_Rc[i - 1 + (j - 1) * shared.dim.vaccinating_into_Rc.mult[1] + (k - 1) * shared.dim.vaccinating_into_Rc.mult[2]] - internal.vaccinating_out_of_Rc[i - 1 + (j - 1) * shared.dim.vaccinating_out_of_Rc.mult[1] + (k - 1) * shared.dim.vaccinating_out_of_Rc.mult[2]] + internal.waning_to_Rc_long[i - 1 + (j - 1) * shared.dim.waning_to_Rc_long.mult[1] + (k - 1) * shared.dim.waning_to_Rc_long.mult[2]] + internal.waning_to_Rc_unvaccinated[i - 1 + (j - 1) * shared.dim.waning_to_Rc_unvaccinated.mult[1] + (k - 1) * shared.dim.waning_to_Rc_unvaccinated.mult[2]] - internal.waning_from_Rc_short[i - 1 + (j - 1) * shared.dim.waning_from_Rc_short.mult[1] + (k - 1) * shared.dim.waning_from_Rc_short.mult[2]] - internal.waning_from_Rc_long[i - 1 + (j - 1) * shared.dim.waning_from_Rc_long.mult[1] + (k - 1) * shared.dim.waning_from_Rc_long.mult[2]], 0);
        }
      }
    }
    state_next[0] = dust2::array::sum<real_type>(internal.vaccination_prop.data(), shared.dim.vaccination_prop);
    state_next[1] = (shared.n_age <= 0 ? 0 : dust2::array::sum<real_type>(internal.ngm.data(), shared.dim.ngm) / shared.n_age);
    state_next[2] = dust2::array::sum<real_type>(internal.seeded_actual.data(), shared.dim.seeded_actual);
    state_next[3] = N;
    for (size_t i = 1; i <= shared.dim.seropositive.size; ++i) {
      state_next[i - 1 + shared.odin.offset.state[14]] = (internal.Npop_age[i - 1] <= 0 ? 0 : (dust2::array::sum<real_type>(S, shared.dim.S, {i - 1, i - 1}, {1, shared.n_vacc - 1}, {0, shared.dim.S.dim[2] - 1}) + dust2::array::sum<real_type>(I, shared.dim.I, {i - 1, i - 1}, {0, shared.dim.I.dim[1] - 1}, {0, shared.dim.I.dim[2] - 1}) + dust2::array::sum<real_type>(Is, shared.dim.Is, {i - 1, i - 1}, {0, shared.dim.Is.dim[1] - 1}, {0, shared.dim.Is.dim[2] - 1}) + dust2::array::sum<real_type>(R, shared.dim.R, {i - 1, i - 1}, {0, shared.dim.R.dim[1] - 1}, {0, shared.dim.R.dim[2] - 1}) + dust2::array::sum<real_type>(Rc, shared.dim.Rc, {i - 1, i - 1}, {0, shared.dim.Rc.dim[1] - 1}, {0, shared.dim.Rc.dim[2] - 1})) / internal.Npop_age[i - 1]);
    }
    state_next[4] = dust2::array::sum<real_type>(internal.Births.data(), shared.dim.Births);
    state_next[5] = dust2::array::sum<real_type>(internal.S_death.data(), shared.dim.S_death) + dust2::array::sum<real_type>(internal.E_death.data(), shared.dim.E_death) + dust2::array::sum<real_type>(internal.I_death.data(), shared.dim.I_death) + dust2::array::sum<real_type>(internal.R_death.data(), shared.dim.R_death) + dust2::array::sum<real_type>(internal.Is_death.data(), shared.dim.Is_death) + dust2::array::sum<real_type>(internal.Rc_death.data(), shared.dim.Rc_death);
    state_next[6] = dust2::array::sum<real_type>(internal.vaccinating_out_of_S.data(), shared.dim.vaccinating_out_of_S);
    state_next[7] = total_births - total_deaths;
    for (size_t i = 1; i <= shared.dim.new_case.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.new_case.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.new_case.dim[2]; ++k) {
          state_next[i - 1 + (j - 1) * shared.dim.new_case.mult[1] + (k - 1) * shared.dim.new_case.mult[2] + shared.odin.offset.state[15]] = internal.incubated[i - 1 + (j - 1) * shared.dim.incubated.mult[1] + (k - 1) * shared.dim.incubated.mult[2]] + internal.seeded_actual[i - 1 + (j - 1) * shared.dim.seeded_actual.mult[1] + (k - 1) * shared.dim.seeded_actual.mult[2]];
        }
      }
    }
  }
  static real_type compare_data(real_type time, const real_type* state, const data_type& data, const shared_state& shared, internal_state& internal, rng_state_type& rng_state) {
    ...
    const auto * seropositive = state + shared.odin.offset.state[14];
    real_type odin_ll = 0;
...
[[cpp11::register]]
SEXP dust2_unfilter_transmission_model_alloc(cpp11::list r_pars, cpp11::sexp r_time_start, cpp11::sexp r_time, cpp11::list r_time_control, cpp11::list r_data, cpp11::sexp r_n_particles, cpp11::sexp r_n_groups, cpp11::sexp r_n_threads) {
  return dust2::r::dust2_discrete_unfilter_alloc<transmission_model>(r_pars, r_time_start, r_time, r_time_control, r_data, r_n_particles, r_n_groups, r_n_threads);
}

[[cpp11::register]]
SEXP dust2_filter_transmission_model_alloc(cpp11::list r_pars, cpp11::sexp r_time_start, cpp11::sexp r_time, cpp11::list r_time_control, cpp11::list r_data, cpp11::sexp r_n_particles, cpp11::sexp r_n_groups, cpp11::sexp r_n_threads, cpp11::sexp r_seed) {
  return dust2::r::dust2_discrete_filter_alloc<transmission_model>(r_pars, r_time_start, r_time, r_time_control, r_data, r_n_particles, r_n_groups, r_n_threads, r_seed);
}
[[cpp11::register]]
SEXP dust2_system_transmission_model_compare_data(cpp11::sexp ptr, cpp11::list r_data, bool preserve_particle_dimension, bool preserve_group_dimension) {
  return dust2::r::dust2_system_compare_data<dust2::dust_discrete<transmission_model>>(ptr, r_data, preserve_particle_dimension, preserve_group_dimension);
}

[[cpp11::register]]
SEXP dust2_unfilter_transmission_model_update_pars(cpp11::sexp ptr, cpp11::list r_pars, cpp11::sexp r_index_group) {
  return dust2::r::dust2_unfilter_update_pars<dust2::dust_discrete<transmission_model>>(ptr, r_pars, r_index_group);
}

[[cpp11::register]]
SEXP dust2_unfilter_transmission_model_run(cpp11::sexp ptr, cpp11::sexp r_initial, bool save_trajectories, cpp11::sexp save_snapshots, bool adjoint, cpp11::sexp r_index_state, cpp11::sexp r_index_group, bool preserve_particle_dimension, bool preserve_group_dimension) {
  return dust2::r::dust2_unfilter_run<dust2::dust_discrete<transmission_model>>(ptr, r_initial, save_trajectories, save_snapshots, adjoint, r_index_state, r_index_group, preserve_particle_dimension, preserve_group_dimension);
}

[[cpp11::register]]
SEXP dust2_unfilter_transmission_model_last_trajectories(cpp11::sexp ptr, bool select_random_particle, bool preserve_particle_dimension, bool preserve_group_dimension) {
  return dust2::r::dust2_unfilter_last_trajectories<dust2::dust_discrete<transmission_model>>(ptr, select_random_particle, preserve_particle_dimension, preserve_group_dimension);
}

[[cpp11::register]]
SEXP dust2_unfilter_transmission_model_last_snapshots(cpp11::sexp ptr, bool select_random_particle, bool preserve_particle_dimension, bool preserve_group_dimension) {
  return dust2::r::dust2_unfilter_last_snapshots<dust2::dust_discrete<transmission_model>>(ptr, select_random_particle, preserve_particle_dimension, preserve_group_dimension);
}

[[cpp11::register]]
SEXP dust2_unfilter_transmission_model_last_state(cpp11::sexp ptr, bool select_random_particle, bool preserve_particle_dimension, bool preserve_group_dimension) {
  return dust2::r::dust2_unfilter_last_state<dust2::dust_discrete<transmission_model>>(ptr, select_random_particle, preserve_particle_dimension, preserve_group_dimension);
}

[[cpp11::register]]
SEXP dust2_filter_transmission_model_update_pars(cpp11::sexp ptr, cpp11::list r_pars, cpp11::sexp r_index_group) {
  return dust2::r::dust2_filter_update_pars<dust2::dust_discrete<transmission_model>>(ptr, r_pars, r_index_group);
}

[[cpp11::register]]
SEXP dust2_filter_transmission_model_run(cpp11::sexp ptr, cpp11::sexp r_initial, bool save_trajectories, cpp11::sexp save_snapshots, bool adjoint, cpp11::sexp index_state, cpp11::sexp index_group, bool preserve_particle_dimension, bool preserve_group_dimension) {
  return dust2::r::dust2_filter_run<dust2::dust_discrete<transmission_model>>(ptr, r_initial, save_trajectories, save_snapshots, adjoint, index_state, index_group, preserve_particle_dimension, preserve_group_dimension);
}

[[cpp11::register]]
SEXP dust2_filter_transmission_model_last_trajectories(cpp11::sexp ptr, bool select_random_particle, bool preserve_particle_dimension, bool preserve_group_dimension) {
  return dust2::r::dust2_filter_last_trajectories<dust2::dust_discrete<transmission_model>>(ptr, select_random_particle, preserve_particle_dimension, preserve_group_dimension);
}

[[cpp11::register]]
SEXP dust2_filter_transmission_model_last_snapshots(cpp11::sexp ptr, bool select_random_particle, bool preserve_particle_dimension, bool preserve_group_dimension) {
  return dust2::r::dust2_filter_last_snapshots<dust2::dust_discrete<transmission_model>>(ptr, select_random_particle, preserve_particle_dimension, preserve_group_dimension);
}

[[cpp11::register]]
SEXP dust2_filter_transmission_model_last_state(cpp11::sexp ptr, bool select_random_particle, bool preserve_particle_dimension, bool preserve_group_dimension) {
  return dust2::r::dust2_filter_last_state<dust2::dust_discrete<transmission_model>>(ptr, select_random_particle, preserve_particle_dimension, preserve_group_dimension);
}

[[cpp11::register]]
SEXP dust2_filter_transmission_model_rng_state(cpp11::sexp ptr) {
  return dust2::r::dust2_filter_rng_state<dust2::dust_discrete<transmission_model>>(ptr);
}

[[cpp11::register]]
SEXP dust2_filter_transmission_model_set_rng_state(cpp11::sexp ptr, cpp11::sexp r_rng_state) {
  return dust2::r::dust2_filter_set_rng_state<dust2::dust_discrete<transmission_model>>(ptr, r_rng_state);
}

...
To do this we use the function `demographic_check()` which will produce three plots. 
...
We can also quickly visualise the outputs using the function `summary_plots()`, which produces four plots and a data frame summarising susceptibility profiles over time and age.
...
- `vaccination_coverage_plot`: A plot of the population level vaccination coverage over time.
...
These plots as depicted for this scenario are as follows:

```{r , echo=FALSE, out.width = '75%', out.height = '75%'}
knitr::include_graphics("vignette_img/vignette_plot_2.jpg", error = FALSE)
```

Time series of all key model compartments: susceptible, exposed, infectious, infectious severe, recovered, recovered with complications, total population, and incident cases. The panels highlight epidemic cycles and long-term transitions in population immunity structure, as well as the demographic trends driving overall population growth.
 
```{r , echo=FALSE, out.width = '75%', out.height = '75%'}
knitr::include_graphics("vignette_img/vignette_plot_3.jpg", error = FALSE)
```

Age-stratified distribution of population immunity at baseline, showing the proportion of each age group classified as susceptible, vaccine-derived protected, infection-derived protected, or with hybrid protection (both vaccine and infection). Younger age groups show a higher proportion of vaccine-derived immunity, while older age groups are dominated by infection-derived protection, reflecting historical vaccination rollout and disease transmission dynamics.

```{r , echo=FALSE, out.width = '75%', out.height = '75%'}
knitr::include_graphics("vignette_img/vignette_plot_4.jpg", error = FALSE)
```

Cumulative number of cases over time as projected by the model. Large epidemics prior to widespread vaccination are followed by a sharp reduction in incident cases once high coverage is achieved, resulting in the flattening of the cumulative curve after the 1990s.

```{r , echo=FALSE, out.width = '75%', out.height = '75%'}
knitr::include_graphics("vignette_img/vignette_plot_5.jpg", error = FALSE)
```

Time series of estimated population-level vaccination coverage from 1950 to present. The rapid increase in coverage during the late 1980s and 1990s reflects scale-up of routine immunisation services, with a subsequent plateau and minor fluctuations in recent years.
...
- `individual_plots`: Which is comprised of

...
public:
    ...
    for (size_t i = 1; i <= shared.dim.S.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.S.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.S.dim[2]; ++k) {
          state[i - 1 + (j - 1) * shared.dim.S.mult[1] + (k - 1) * shared.dim.S.mult[2] + 16] = shared.S0[i - 1 + (j - 1) * shared.dim.S0.mult[1] + (k - 1) * shared.dim.S0.mult[2]];
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.E.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.E.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.E.dim[2]; ++k) {
          state[i - 1 + (j - 1) * shared.dim.E.mult[1] + (k - 1) * shared.dim.E.mult[2] + shared.odin.offset.state[17]] = 0;
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.I.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.I.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.I.dim[2]; ++k) {
          state[i - 1 + (j - 1) * shared.dim.I.mult[1] + (k - 1) * shared.dim.I.mult[2] + shared.odin.offset.state[18]] = shared.I0[i - 1 + (j - 1) * shared.dim.I0.mult[1] + (k - 1) * shared.dim.I0.mult[2]];
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.R.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.R.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.R.dim[2]; ++k) {
          state[i - 1 + (j - 1) * shared.dim.R.mult[1] + (k - 1) * shared.dim.R.mult[2] + shared.odin.offset.state[19]] = shared.Rpop0[i - 1 + (j - 1) * shared.dim.Rpop0.mult[1] + (k - 1) * shared.dim.Rpop0.mult[2]];
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.Is.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.Is.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.Is.dim[2]; ++k) {
          state[i - 1 + (j - 1) * shared.dim.Is.mult[1] + (k - 1) * shared.dim.Is.mult[2] + shared.odin.offset.state[20]] = 0;
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.Rc.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.Rc.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.Rc.dim[2]; ++k) {
          state[i - 1 + (j - 1) * shared.dim.Rc.mult[1] + (k - 1) * shared.dim.Rc.mult[2] + shared.odin.offset.state[21]] = 0;
        }
      }
    }
    ...
    for (size_t i = 1; i <= shared.dim.new_case.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.new_case.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.new_case.dim[2]; ++k) {
          state[i - 1 + (j - 1) * shared.dim.new_case.mult[1] + (k - 1) * shared.dim.new_case.mult[2] + shared.odin.offset.state[23]] = shared.I0[i - 1 + (j - 1) * shared.dim.I0.mult[1] + (k - 1) * shared.dim.I0.mult[2]];
        }
      }
    }
    state[3] = shared.R0[0];
  ...
  static void update(real_type time, real_type dt, const real_type* state, const shared_state& shared, internal_state& internal, rng_state_type& rng_state, real_type* state_next) {
    const auto * S = state + 16;
    const auto * E = state + shared.odin.offset.state[17];
    const auto * I = state + shared.odin.offset.state[18];
    const auto * R = state + shared.odin.offset.state[19];
    const auto * Is = state + shared.odin.offset.state[20];
    const auto * Rc = state + shared.odin.offset.state[21];
    const auto total_deaths = state[5];
    const auto total_births = state[6];
    const auto total_vaccinated_out = state[7];
    const auto total_vaccinated_in = state[8];
    ...
    for (size_t i = 1; i <= shared.dim.S.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.S.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.S.dim[2]; ++k) {
          state_next[i - 1 + (j - 1) * shared.dim.S.mult[1] + (k - 1) * shared.dim.S.mult[2] + 16] = monty::math::max(S[i - 1 + (j - 1) * shared.dim.S.mult[1] + (k - 1) * shared.dim.S.mult[2]] + internal.waning_R[i - 1 + (j - 1) * shared.dim.waning_R.mult[1] + (k - 1) * shared.dim.waning_R.mult[2]] + internal.waning_Rc[i - 1 + (j - 1) * shared.dim.waning_Rc.mult[1] + (k - 1) * shared.dim.waning_Rc.mult[2]] + internal.aging_into_S[i - 1 + (j - 1) * shared.dim.aging_into_S.mult[1] + (k - 1) * shared.dim.aging_into_S.mult[2]] - internal.aging_out_of_S[i - 1 + (j - 1) * shared.dim.aging_out_of_S.mult[1] + (k - 1) * shared.dim.aging_out_of_S.mult[2]] - internal.lambda_S[i - 1 + (j - 1) * shared.dim.lambda_S.mult[1] + (k - 1) * shared.dim.lambda_S.mult[2]] - internal.S_death[i - 1 + (j - 1) * shared.dim.S_death.mult[1] + (k - 1) * shared.dim.S_death.mult[2]] + internal.migration_S[i - 1 + (j - 1) * shared.dim.migration_S.mult[1] + (k - 1) * shared.dim.migration_S.mult[2]] * pos_neg_migration + internal.vaccinating_into_S[i - 1 + (j - 1) * shared.dim.vaccinating_into_S.mult[1] + (k - 1) * shared.dim.vaccinating_into_S.mult[2]] - internal.vaccinating_out_of_S[i - 1 + (j - 1) * shared.dim.vaccinating_out_of_S.mult[1] + (k - 1) * shared.dim.vaccinating_out_of_S.mult[2]] + internal.waning_to_S_long[i - 1 + (j - 1) * shared.dim.waning_to_S_long.mult[1] + (k - 1) * shared.dim.waning_to_S_long.mult[2]] + internal.waning_to_S_unvaccinated[i - 1 + (j - 1) * shared.dim.waning_to_S_unvaccinated.mult[1] + (k - 1) * shared.dim.waning_to_S_unvaccinated.mult[2]] - internal.waning_from_S_short[i - 1 + (j - 1) * shared.dim.waning_from_S_short.mult[1] + (k - 1) * shared.dim.waning_from_S_short.mult[2]] - internal.waning_from_S_long[i - 1 + (j - 1) * shared.dim.waning_from_S_long.mult[1] + (k - 1) * shared.dim.waning_from_S_long.mult[2]] - internal.seeded_actual[i - 1 + (j - 1) * shared.dim.seeded_actual.mult[1] + (k - 1) * shared.dim.seeded_actual.mult[2]] + internal.waning_to_S_short[i - 1 + (j - 1) * shared.dim.waning_to_S_short.mult[1] + (k - 1) * shared.dim.waning_to_S_short.mult[2]], static_cast<real_type>(0));
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.E.dim[0]; ++i) {
    ...
    for (size_t i = 1; i <= shared.dim.new_case.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.new_case.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.new_case.dim[2]; ++k) {
          state_next[i - 1 + (j - 1) * shared.dim.new_case.mult[1] + (k - 1) * shared.dim.new_case.mult[2] + shared.odin.offset.state[23]] = internal.incubated[i - 1 + (j - 1) * shared.dim.incubated.mult[1] + (k - 1) * shared.dim.incubated.mult[2]] + internal.t_seeded[i - 1 + (j - 1) * shared.dim.t_seeded.mult[1] + (k - 1) * shared.dim.t_seeded.mult[2]];
        }
      }
    }
    state_next[3] = (shared.n_age <= 0 ? 0 : dust2::array::sum<real_type>(internal.ngm.data(), shared.dim.ngm) / shared.n_age);
    state_next[4] = dust2::array::sum<real_type>(internal.vaccination_prop.data(), shared.dim.vaccination_prop);
    ...
    state_next[15] = dust2::array::sum<real_type>(internal.reproductive_population.data(), shared.dim.reproductive_population);
  }
};

...
public:
  ...
  static void update(real_type time, real_type dt, const real_type* state, const shared_state& shared, internal_state& internal, rng_state_type& rng_state, real_type* state_next) {
    ...
    for (size_t i = 1; i <= static_cast<size_t>((shared.n_age - 1)); ++i) {
      for (size_t j = 1; j <= shared.dim.aging_out_of_S.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.aging_out_of_S.dim[2]; ++k) {
          internal.aging_out_of_S[i - 1 + (j - 1) * shared.dim.aging_out_of_S.mult[1] + (k - 1) * shared.dim.aging_out_of_S.mult[2]] = S[i - 1 + (j - 1) * shared.dim.S.mult[1] + (k - 1) * shared.dim.S.mult[2]] * monty::math::max(monty::math::min(shared.aging_rate[i - 1], static_cast<real_type>(1)), static_cast<real_type>(0));
        }
      }
    }
    for (size_t i = 2; i <= static_cast<size_t>(shared.n_age); ++i) {
      for (size_t j = 1; j <= shared.dim.aging_into_E.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.aging_into_E.dim[2]; ++k) {
          internal.aging_into_E[i - 1 + (j - 1) * shared.dim.aging_into_E.mult[1] + (k - 1) * shared.dim.aging_into_E.mult[2]] = E[i - 1 - 1 + (j - 1) * shared.dim.E.mult[1] + (k - 1) * shared.dim.E.mult[2]] * monty::math::max(monty::math::min(shared.aging_rate[i - 1 - 1], static_cast<real_type>(1)), static_cast<real_type>(0));
        }
      }
    }
    for (size_t i = 1; i <= static_cast<size_t>((shared.n_age - 1)); ++i) {
      for (size_t j = 1; j <= shared.dim.aging_out_of_E.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.aging_out_of_E.dim[2]; ++k) {
          internal.aging_out_of_E[i - 1 + (j - 1) * shared.dim.aging_out_of_E.mult[1] + (k - 1) * shared.dim.aging_out_of_E.mult[2]] = E[i - 1 + (j - 1) * shared.dim.E.mult[1] + (k - 1) * shared.dim.E.mult[2]] * monty::math::max(monty::math::min(shared.aging_rate[i - 1], static_cast<real_type>(1)), static_cast<real_type>(0));
        }
      }
    }
    for (size_t i = 2; i <= static_cast<size_t>(shared.n_age); ++i) {
      for (size_t j = 1; j <= shared.dim.aging_into_I.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.aging_into_I.dim[2]; ++k) {
          internal.aging_into_I[i - 1 + (j - 1) * shared.dim.aging_into_I.mult[1] + (k - 1) * shared.dim.aging_into_I.mult[2]] = I[i - 1 - 1 + (j - 1) * shared.dim.I.mult[1] + (k - 1) * shared.dim.I.mult[2]] * monty::math::max(monty::math::min(shared.aging_rate[i - 1 - 1], static_cast<real_type>(1)), static_cast<real_type>(0));
        }
      }
    }
    for (size_t i = 1; i <= static_cast<size_t>((shared.n_age - 1)); ++i) {
      for (size_t j = 1; j <= shared.dim.aging_out_of_I.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.aging_out_of_I.dim[2]; ++k) {
          internal.aging_out_of_I[i - 1 + (j - 1) * shared.dim.aging_out_of_I.mult[1] + (k - 1) * shared.dim.aging_out_of_I.mult[2]] = I[i - 1 + (j - 1) * shared.dim.I.mult[1] + (k - 1) * shared.dim.I.mult[2]] * monty::math::max(monty::math::min(shared.aging_rate[i - 1], static_cast<real_type>(1)), static_cast<real_type>(0));
        }
      }
    }
    for (size_t i = 2; i <= static_cast<size_t>(shared.n_age); ++i) {
      for (size_t j = 1; j <= shared.dim.aging_into_R.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.aging_into_R.dim[2]; ++k) {
          internal.aging_into_R[i - 1 + (j - 1) * shared.dim.aging_into_R.mult[1] + (k - 1) * shared.dim.aging_into_R.mult[2]] = R[i - 1 - 1 + (j - 1) * shared.dim.R.mult[1] + (k - 1) * shared.dim.R.mult[2]] * monty::math::max(monty::math::min(shared.aging_rate[i - 1 - 1], static_cast<real_type>(1)), static_cast<real_type>(0));
        }
      }
    }
    for (size_t i = 1; i <= static_cast<size_t>((shared.n_age - 1)); ++i) {
      for (size_t j = 1; j <= shared.dim.aging_out_of_R.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.aging_out_of_R.dim[2]; ++k) {
          internal.aging_out_of_R[i - 1 + (j - 1) * shared.dim.aging_out_of_R.mult[1] + (k - 1) * shared.dim.aging_out_of_R.mult[2]] = R[i - 1 + (j - 1) * shared.dim.R.mult[1] + (k - 1) * shared.dim.R.mult[2]] * monty::math::max(monty::math::min(shared.aging_rate[i - 1], static_cast<real_type>(1)), static_cast<real_type>(0));
        }
      }
    }
    for (size_t i = 2; i <= static_cast<size_t>(shared.n_age); ++i) {
      for (size_t j = 1; j <= shared.dim.aging_into_Is.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.aging_into_Is.dim[2]; ++k) {
          internal.aging_into_Is[i - 1 + (j - 1) * shared.dim.aging_into_Is.mult[1] + (k - 1) * shared.dim.aging_into_Is.mult[2]] = Is[i - 1 - 1 + (j - 1) * shared.dim.Is.mult[1] + (k - 1) * shared.dim.Is.mult[2]] * monty::math::max(monty::math::min(shared.aging_rate[i - 1 - 1], static_cast<real_type>(1)), static_cast<real_type>(0));
        }
      }
    }
    for (size_t i = 1; i <= static_cast<size_t>((shared.n_age - 1)); ++i) {
      for (size_t j = 1; j <= shared.dim.aging_out_of_Is.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.aging_out_of_Is.dim[2]; ++k) {
          internal.aging_out_of_Is[i - 1 + (j - 1) * shared.dim.aging_out_of_Is.mult[1] + (k - 1) * shared.dim.aging_out_of_Is.mult[2]] = Is[i - 1 + (j - 1) * shared.dim.Is.mult[1] + (k - 1) * shared.dim.Is.mult[2]] * monty::math::max(monty::math::min(shared.aging_rate[i - 1], static_cast<real_type>(1)), static_cast<real_type>(0));
        }
      }
    }
    for (size_t i = 2; i <= static_cast<size_t>(shared.n_age); ++i) {
      for (size_t j = 1; j <= shared.dim.aging_into_Rc.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.aging_into_Rc.dim[2]; ++k) {
          internal.aging_into_Rc[i - 1 + (j - 1) * shared.dim.aging_into_Rc.mult[1] + (k - 1) * shared.dim.aging_into_Rc.mult[2]] = Rc[i - 1 - 1 + (j - 1) * shared.dim.Rc.mult[1] + (k - 1) * shared.dim.Rc.mult[2]] * monty::math::max(monty::math::min(shared.aging_rate[i - 1 - 1], static_cast<real_type>(1)), static_cast<real_type>(0));
        }
      }
    }
    for (size_t i = 1; i <= static_cast<size_t>((shared.n_age - 1)); ++i) {
      for (size_t j = 1; j <= shared.dim.aging_out_of_Rc.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.aging_out_of_Rc.dim[2]; ++k) {
          internal.aging_out_of_Rc[i - 1 + (j - 1) * shared.dim.aging_out_of_Rc.mult[1] + (k - 1) * shared.dim.aging_out_of_Rc.mult[2]] = Rc[i - 1 + (j - 1) * shared.dim.Rc.mult[1] + (k - 1) * shared.dim.Rc.mult[2]] * monty::math::max(monty::math::min(shared.aging_rate[i - 1], static_cast<real_type>(1)), static_cast<real_type>(0));
        }
      }
    }
    ...
    for (size_t i = 1; i <= shared.dim.vaccinating_out_of_E.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.vaccinating_out_of_E.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.vaccinating_out_of_E.dim[2]; ++k) {
          internal.vaccinating_out_of_E[i - 1 + (j - 1) * shared.dim.vaccinating_out_of_E.mult[1] + (k - 1) * shared.dim.vaccinating_out_of_E.mult[2]] = ((j == 1 && shared.n_vacc < 3) || (std::fmod(j, 2) == 0 && j + 3 > shared.n_vacc) || (std::fmod(j, 2) == 1 && j > 1 && j + 2 > shared.n_vacc) || internal.E_after_aging[i - 1 + (j - 1) * shared.dim.E_after_aging.mult[1] + (k - 1) * shared.dim.E_after_aging.mult[2]] <= 0 || internal.vaccination_prop[i - 1 + (j - 1) * shared.dim.vaccination_prop.mult[1] + (k - 1) * shared.dim.vaccination_prop.mult[2]] <= 0 ? 0 : (shared.stochastic_vaccination == 1 ? monty::random::binomial<real_type>(rng_state, internal.E_after_aging[i - 1 + (j - 1) * shared.dim.E_after_aging.mult[1] + (k - 1) * shared.dim.E_after_aging.mult[2]], monty::math::max(monty::math::min(internal.vaccination_prop[i - 1 + (j - 1) * shared.dim.vaccination_prop.mult[1] + (k - 1) * shared.dim.vaccination_prop.mult[2]], static_cast<real_type>(1)), static_cast<real_type>(0))) : internal.E_after_aging[i - 1 + (j - 1) * shared.dim.E_after_aging.mult[1] + (k - 1) * shared.dim.E_after_aging.mult[2]] * monty::math::max(monty::math::min(internal.vaccination_prop[i - 1 + (j - 1) * shared.dim.vaccination_prop.mult[1] + (k - 1) * shared.dim.vaccination_prop.mult[2]], static_cast<real_type>(1)), static_cast<real_type>(0))));
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.vaccinating_out_of_I.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.vaccinating_out_of_I.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.vaccinating_out_of_I.dim[2]; ++k) {
          internal.vaccinating_out_of_I[i - 1 + (j - 1) * shared.dim.vaccinating_out_of_I.mult[1] + (k - 1) * shared.dim.vaccinating_out_of_I.mult[2]] = ((j == 1 && shared.n_vacc < 3) || (std::fmod(j, 2) == 0 && j + 3 > shared.n_vacc) || (std::fmod(j, 2) == 1 && j > 1 && j + 2 > shared.n_vacc) || internal.I_after_aging[i - 1 + (j - 1) * shared.dim.I_after_aging.mult[1] + (k - 1) * shared.dim.I_after_aging.mult[2]] <= 0 || internal.vaccination_prop[i - 1 + (j - 1) * shared.dim.vaccination_prop.mult[1] + (k - 1) * shared.dim.vaccination_prop.mult[2]] <= 0 ? 0 : (shared.stochastic_vaccination == 1 ? monty::random::binomial<real_type>(rng_state, internal.I_after_aging[i - 1 + (j - 1) * shared.dim.I_after_aging.mult[1] + (k - 1) * shared.dim.I_after_aging.mult[2]], monty::math::max(monty::math::min(internal.vaccination_prop[i - 1 + (j - 1) * shared.dim.vaccination_prop.mult[1] + (k - 1) * shared.dim.vaccination_prop.mult[2]], static_cast<real_type>(1)), static_cast<real_type>(0))) : internal.I_after_aging[i - 1 + (j - 1) * shared.dim.I_after_aging.mult[1] + (k - 1) * shared.dim.I_after_aging.mult[2]] * monty::math::max(monty::math::min(internal.vaccination_prop[i - 1 + (j - 1) * shared.dim.vaccination_prop.mult[1] + (k - 1) * shared.dim.vaccination_prop.mult[2]], static_cast<real_type>(1)), static_cast<real_type>(0))));
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.vaccinating_out_of_R.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.vaccinating_out_of_R.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.vaccinating_out_of_R.dim[2]; ++k) {
          internal.vaccinating_out_of_R[i - 1 + (j - 1) * shared.dim.vaccinating_out_of_R.mult[1] + (k - 1) * shared.dim.vaccinating_out_of_R.mult[2]] = ((j == 1 && shared.n_vacc < 3) || (std::fmod(j, 2) == 0 && j + 3 > shared.n_vacc) || (std::fmod(j, 2) == 1 && j > 1 && j + 2 > shared.n_vacc) || internal.R_after_aging[i - 1 + (j - 1) * shared.dim.R_after_aging.mult[1] + (k - 1) * shared.dim.R_after_aging.mult[2]] <= 0 || internal.vaccination_prop[i - 1 + (j - 1) * shared.dim.vaccination_prop.mult[1] + (k - 1) * shared.dim.vaccination_prop.mult[2]] <= 0 ? 0 : (shared.stochastic_vaccination == 1 ? monty::random::binomial<real_type>(rng_state, internal.R_after_aging[i - 1 + (j - 1) * shared.dim.R_after_aging.mult[1] + (k - 1) * shared.dim.R_after_aging.mult[2]], monty::math::max(monty::math::min(internal.vaccination_prop[i - 1 + (j - 1) * shared.dim.vaccination_prop.mult[1] + (k - 1) * shared.dim.vaccination_prop.mult[2]], static_cast<real_type>(1)), static_cast<real_type>(0))) : internal.R_after_aging[i - 1 + (j - 1) * shared.dim.R_after_aging.mult[1] + (k - 1) * shared.dim.R_after_aging.mult[2]] * monty::math::max(monty::math::min(internal.vaccination_prop[i - 1 + (j - 1) * shared.dim.vaccination_prop.mult[1] + (k - 1) * shared.dim.vaccination_prop.mult[2]], static_cast<real_type>(1)), static_cast<real_type>(0))));
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.vaccinating_out_of_Is.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.vaccinating_out_of_Is.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.vaccinating_out_of_Is.dim[2]; ++k) {
          internal.vaccinating_out_of_Is[i - 1 + (j - 1) * shared.dim.vaccinating_out_of_Is.mult[1] + (k - 1) * shared.dim.vaccinating_out_of_Is.mult[2]] = ((j == 1 && shared.n_vacc < 3) || (std::fmod(j, 2) == 0 && j + 3 > shared.n_vacc) || (std::fmod(j, 2) == 1 && j > 1 && j + 2 > shared.n_vacc) || internal.Is_after_aging[i - 1 + (j - 1) * shared.dim.Is_after_aging.mult[1] + (k - 1) * shared.dim.Is_after_aging.mult[2]] <= 0 || internal.vaccination_prop[i - 1 + (j - 1) * shared.dim.vaccination_prop.mult[1] + (k - 1) * shared.dim.vaccination_prop.mult[2]] <= 0 ? 0 : (shared.stochastic_vaccination == 1 ? monty::random::binomial<real_type>(rng_state, internal.Is_after_aging[i - 1 + (j - 1) * shared.dim.Is_after_aging.mult[1] + (k - 1) * shared.dim.Is_after_aging.mult[2]], monty::math::max(monty::math::min(internal.vaccination_prop[i - 1 + (j - 1) * shared.dim.vaccination_prop.mult[1] + (k - 1) * shared.dim.vaccination_prop.mult[2]], static_cast<real_type>(1)), static_cast<real_type>(0))) : internal.Is_after_aging[i - 1 + (j - 1) * shared.dim.Is_after_aging.mult[1] + (k - 1) * shared.dim.Is_after_aging.mult[2]] * monty::math::max(monty::math::min(internal.vaccination_prop[i - 1 + (j - 1) * shared.dim.vaccination_prop.mult[1] + (k - 1) * shared.dim.vaccination_prop.mult[2]], static_cast<real_type>(1)), static_cast<real_type>(0))));
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.vaccinating_out_of_Rc.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.vaccinating_out_of_Rc.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.vaccinating_out_of_Rc.dim[2]; ++k) {
          internal.vaccinating_out_of_Rc[i - 1 + (j - 1) * shared.dim.vaccinating_out_of_Rc.mult[1] + (k - 1) * shared.dim.vaccinating_out_of_Rc.mult[2]] = ((j == 1 && shared.n_vacc < 3) || (std::fmod(j, 2) == 0 && j + 3 > shared.n_vacc) || (std::fmod(j, 2) == 1 && j > 1 && j + 2 > shared.n_vacc) || internal.Rc_after_aging[i - 1 + (j - 1) * shared.dim.Rc_after_aging.mult[1] + (k - 1) * shared.dim.Rc_after_aging.mult[2]] <= 0 || internal.vaccination_prop[i - 1 + (j - 1) * shared.dim.vaccination_prop.mult[1] + (k - 1) * shared.dim.vaccination_prop.mult[2]] <= 0 ? 0 : (shared.stochastic_vaccination == 1 ? monty::random::binomial<real_type>(rng_state, internal.Rc_after_aging[i - 1 + (j - 1) * shared.dim.Rc_after_aging.mult[1] + (k - 1) * shared.dim.Rc_after_aging.mult[2]], monty::math::max(monty::math::min(internal.vaccination_prop[i - 1 + (j - 1) * shared.dim.vaccination_prop.mult[1] + (k - 1) * shared.dim.vaccination_prop.mult[2]], static_cast<real_type>(1)), static_cast<real_type>(0))) : internal.Rc_after_aging[i - 1 + (j - 1) * shared.dim.Rc_after_aging.mult[1] + (k - 1) * shared.dim.Rc_after_aging.mult[2]] * monty::math::max(monty::math::min(internal.vaccination_prop[i - 1 + (j - 1) * shared.dim.vaccination_prop.mult[1] + (k - 1) * shared.dim.vaccination_prop.mult[2]], static_cast<real_type>(1)), static_cast<real_type>(0))));
        }
      }
    }
    ...
    for (size_t i = 1; i <= shared.dim.vaccinating_into_E.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.vaccinating_into_E.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.vaccinating_into_E.dim[2]; ++k) {
          internal.vaccinating_into_E[i - 1 + (j - 1) * shared.dim.vaccinating_into_E.mult[1] + (k - 1) * shared.dim.vaccinating_into_E.mult[2]] = (j == 3 ? internal.vaccinating_out_of_E[i - 1 + (k - 1) * shared.dim.vaccinating_out_of_E.mult[2]] : (j > 3 && std::fmod(j, 2) == 1 ? internal.vaccinating_out_of_E[i - 1 + (j - 2 - 1) * shared.dim.vaccinating_out_of_E.mult[1] + (k - 1) * shared.dim.vaccinating_out_of_E.mult[2]] + internal.vaccinating_out_of_E[i - 1 + (j - 3 - 1) * shared.dim.vaccinating_out_of_E.mult[1] + (k - 1) * shared.dim.vaccinating_out_of_E.mult[2]] : 0));
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.vaccinating_into_I.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.vaccinating_into_I.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.vaccinating_into_I.dim[2]; ++k) {
          internal.vaccinating_into_I[i - 1 + (j - 1) * shared.dim.vaccinating_into_I.mult[1] + (k - 1) * shared.dim.vaccinating_into_I.mult[2]] = (j == 3 ? internal.vaccinating_out_of_I[i - 1 + (k - 1) * shared.dim.vaccinating_out_of_I.mult[2]] : (j > 3 && std::fmod(j, 2) == 1 ? internal.vaccinating_out_of_I[i - 1 + (j - 2 - 1) * shared.dim.vaccinating_out_of_I.mult[1] + (k - 1) * shared.dim.vaccinating_out_of_I.mult[2]] + internal.vaccinating_out_of_I[i - 1 + (j - 3 - 1) * shared.dim.vaccinating_out_of_I.mult[1] + (k - 1) * shared.dim.vaccinating_out_of_I.mult[2]] : 0));
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.vaccinating_into_R.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.vaccinating_into_R.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.vaccinating_into_R.dim[2]; ++k) {
          internal.vaccinating_into_R[i - 1 + (j - 1) * shared.dim.vaccinating_into_R.mult[1] + (k - 1) * shared.dim.vaccinating_into_R.mult[2]] = (j == 3 ? internal.vaccinating_out_of_R[i - 1 + (k - 1) * shared.dim.vaccinating_out_of_R.mult[2]] : (j > 3 && std::fmod(j, 2) == 1 ? internal.vaccinating_out_of_R[i - 1 + (j - 2 - 1) * shared.dim.vaccinating_out_of_R.mult[1] + (k - 1) * shared.dim.vaccinating_out_of_R.mult[2]] + internal.vaccinating_out_of_R[i - 1 + (j - 3 - 1) * shared.dim.vaccinating_out_of_R.mult[1] + (k - 1) * shared.dim.vaccinating_out_of_R.mult[2]] : 0));
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.vaccinating_into_Is.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.vaccinating_into_Is.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.vaccinating_into_Is.dim[2]; ++k) {
          internal.vaccinating_into_Is[i - 1 + (j - 1) * shared.dim.vaccinating_into_Is.mult[1] + (k - 1) * shared.dim.vaccinating_into_Is.mult[2]] = (j == 3 ? internal.vaccinating_out_of_Is[i - 1 + (k - 1) * shared.dim.vaccinating_out_of_Is.mult[2]] : (j > 3 && std::fmod(j, 2) == 1 ? internal.vaccinating_out_of_Is[i - 1 + (j - 2 - 1) * shared.dim.vaccinating_out_of_Is.mult[1] + (k - 1) * shared.dim.vaccinating_out_of_Is.mult[2]] + internal.vaccinating_out_of_Is[i - 1 + (j - 3 - 1) * shared.dim.vaccinating_out_of_Is.mult[1] + (k - 1) * shared.dim.vaccinating_out_of_Is.mult[2]] : 0));
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.vaccinating_into_Rc.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.vaccinating_into_Rc.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.vaccinating_into_Rc.dim[2]; ++k) {
          internal.vaccinating_into_Rc[i - 1 + (j - 1) * shared.dim.vaccinating_into_Rc.mult[1] + (k - 1) * shared.dim.vaccinating_into_Rc.mult[2]] = (j == 3 ? internal.vaccinating_out_of_Rc[i - 1 + (k - 1) * shared.dim.vaccinating_out_of_Rc.mult[2]] : (j > 3 && std::fmod(j, 2) == 1 ? internal.vaccinating_out_of_Rc[i - 1 + (j - 2 - 1) * shared.dim.vaccinating_out_of_Rc.mult[1] + (k - 1) * shared.dim.vaccinating_out_of_Rc.mult[2]] + internal.vaccinating_out_of_Rc[i - 1 + (j - 3 - 1) * shared.dim.vaccinating_out_of_Rc.mult[1] + (k - 1) * shared.dim.vaccinating_out_of_Rc.mult[2]] : 0));
        }
      }
    }
    ...
    for (size_t i = 1; i <= shared.dim.Births.size; ++i) {
      internal.Births[i - 1] = (internal.reproductive_population[i - 1] <= 0 ? 0 : (shared.simp_birth_death == 1 ? monty::random::binomial<real_type>(rng_state, internal.reproductive_population[i - 1], monty::math::max(monty::math::min(internal.birth_rate[i - 1] / 2, static_cast<real_type>(1)), static_cast<real_type>(0))) : (shared.stochastic_birth == 1 ? monty::random::binomial<real_type>(rng_state, internal.reproductive_population[i - 1], monty::math::max(monty::math::min(internal.birth_int[i - 1] / 2, static_cast<real_type>(1)), static_cast<real_type>(0))) : internal.reproductive_population[i - 1] * monty::math::max(monty::math::min(internal.birth_int[i - 1] / 2, static_cast<real_type>(1)), static_cast<real_type>(0)))));
    }
    ...
    for (size_t i = 2; i <= static_cast<size_t>(shared.n_age); ++i) {
      for (size_t j = 1; j <= shared.dim.aging_into_S.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.aging_into_S.dim[2]; ++k) {
          internal.aging_into_S[i - 1 + (j - 1) * shared.dim.aging_into_S.mult[1] + (k - 1) * shared.dim.aging_into_S.mult[2]] = S[i - 1 - 1 + (j - 1) * shared.dim.S.mult[1] + (k - 1) * shared.dim.S.mult[2]] * monty::math::max(monty::math::min(shared.aging_rate[i - 1 - 1], static_cast<real_type>(1)), static_cast<real_type>(0));
        }
      }
    }
    ...
    for (size_t i = 1; i <= shared.dim.vaccinating_out_of_S.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.vaccinating_out_of_S.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.vaccinating_out_of_S.dim[2]; ++k) {
          internal.vaccinating_out_of_S[i - 1 + (j - 1) * shared.dim.vaccinating_out_of_S.mult[1] + (k - 1) * shared.dim.vaccinating_out_of_S.mult[2]] = (shared.n_vacc == 1 || j >= shared.n_vacc - 1 || internal.S_after_aging[i - 1 + (j - 1) * shared.dim.S_after_aging.mult[1] + (k - 1) * shared.dim.S_after_aging.mult[2]] <= 0 || internal.vaccination_prop[i - 1 + (j - 1) * shared.dim.vaccination_prop.mult[1] + (k - 1) * shared.dim.vaccination_prop.mult[2]] <= 0 ? 0 : (shared.stochastic_vaccination == 1 ? monty::random::binomial<real_type>(rng_state, internal.S_after_aging[i - 1 + (j - 1) * shared.dim.S_after_aging.mult[1] + (k - 1) * shared.dim.S_after_aging.mult[2]], monty::math::max(monty::math::min(internal.vaccination_prop[i - 1 + (j - 1) * shared.dim.vaccination_prop.mult[1] + (k - 1) * shared.dim.vaccination_prop.mult[2]], static_cast<real_type>(1)), static_cast<real_type>(0))) : internal.S_after_aging[i - 1 + (j - 1) * shared.dim.S_after_aging.mult[1] + (k - 1) * shared.dim.S_after_aging.mult[2]] * monty::math::max(monty::math::min(internal.vaccination_prop[i - 1 + (j - 1) * shared.dim.vaccination_prop.mult[1] + (k - 1) * shared.dim.vaccination_prop.mult[2]], static_cast<real_type>(1)), static_cast<real_type>(0))));
        }
      }
    }
    ...
    for (size_t i = 1; i <= shared.dim.S.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.S.dim[1]; ++j) {
        for (size_t k = 1; k <= shared.dim.S.dim[2]; ++k) {
          state_next[i - 1 + (j - 1) * shared.dim.S.mult[1] + (k - 1) * shared.dim.S.mult[2] + 17] = monty::math::max(S[i - 1 + (j - 1) * shared.dim.S.mult[1] + (k - 1) * shared.dim.S.mult[2]] + internal.waning_R[i - 1 + (j - 1) * shared.dim.waning_R.mult[1] + (k - 1) * shared.dim.waning_R.mult[2]] + internal.waning_Rc[i - 1 + (j - 1) * shared.dim.waning_Rc.mult[1] + (k - 1) * shared.dim.waning_Rc.mult[2]] + internal.aging_into_S[i - 1 + (j - 1) * shared.dim.aging_into_S.mult[1] + (k - 1) * shared.dim.aging_into_S.mult[2]] - internal.aging_out_of_S[i - 1 + (j - 1) * shared.dim.aging_out_of_S.mult[1] + (k - 1) * shared.dim.aging_out_of_S.mult[2]] - internal.lambda_S[i - 1 + (j - 1) * shared.dim.lambda_S.mult[1] + (k - 1) * shared.dim.lambda_S.mult[2]] - internal.S_death[i - 1 + (j - 1) * shared.dim.S_death.mult[1] + (k - 1) * shared.dim.S_death.mult[2]] + internal.migration_S[i - 1 + (j - 1) * shared.dim.migration_S.mult[1] + (k - 1) * shared.dim.migration_S.mult[2]] * pos_neg_migration + internal.vaccinating_into_S[i - 1 + (j - 1) * shared.dim.vaccinating_into_S.mult[1] + (k - 1) * shared.dim.vaccinating_into_S.mult[2]] - internal.vaccinating_out_of_S[i - 1 + (j - 1) * shared.dim.vaccinating_out_of_S.mult[1] + (k - 1) * shared.dim.vaccinating_out_of_S.mult[2]] + internal.waning_to_S_long[i - 1 + (j - 1) * shared.dim.waning_to_S_long.mult[1] + (k - 1) * shared.dim.waning_to_S_long.mult[2]] + internal.waning_to_S_unvaccinated[i - 1 + (j - 1) * shared.dim.waning_to_S_unvaccinated.mult[1] + (k - 1) * shared.dim.waning_to_S_unvaccinated.mult[2]] - internal.waning_from_S_short[i - 1 + (j - 1) * shared.dim.waning_from_S_short.mult[1] + (k - 1) * shared.dim.waning_from_S_short.mult[2]] - internal.waning_from_S_long[i - 1 + (j - 1) * shared.dim.waning_from_S_long.mult[1] + (k - 1) * shared.dim.waning_from_S_long.mult[2]] - internal.seeded_actual[i - 1 + (j - 1) * shared.dim.seeded_actual.mult[1] + (k - 1) * shared.dim.seeded_actual.mult[2]] + internal.waning_to_S_short[i - 1 + (j - 1) * shared.dim.waning_to_S_short.mult[1] + (k - 1) * shared.dim.waning_to_S_short.mult[2]], static_cast<real_type>(0));
        }
      }
    }
    for (size_t i = 1; i <= shared.dim.E.dim[0]; ++i) {
